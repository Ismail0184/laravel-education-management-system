...
<div class="row justify-content-center pt-5">
    <div class="col-lg-11">
        <div class="col-lg-6 col-5 text-right">
            <a href="{{ route('months.index') }}" class="btn btn-sm btn-neutral">{{ __('lang.refresh') }}</a>
            <a href="{{ route('months.create') }}" class="btn btn-sm btn-neutral">{{ __('lang.addnew') }}</a>
            <a href="{{ route('months.report') }}" class="btn btn-sm btn-neutral">{{ __('lang.report') }}</a>
        </div>
    @if (count($errors) > 0)
            ...
            <table class="table align-items-center table-flush">
                <thead class="thead-light">
                    <tr >
                        ...
                        <th scope="col" class="text-orange w-60">{{ __('lang.monthname') }}</th>
                ...
                <tbody>
                    ...
                    <td class="font-weight-bold text-blue text-center">{{ \App\Http\Controllers\BnumberController::toBangla(++$i) }}</td> 
                        ...
                        <td class="font-weight-bold text-blue">{{ \App\Http\Controllers\BnumberController::toBangla($data->days) }}</td>
                        ...
                        <td class="text-right">
                            <a href="{{ route('months.show', $data->id) }}" title="show">
                                <i class="fas fa-eye text-orange  fa-lg"></i>
                            </a>
                           <a href="{{ route('months.edit', $data->id) }}">
                                <i class="fa fas fa-edit  fa-lg"></i>

                            </a>
                            <a data-toggle="modal" id="smallButton" data-target="#smallModal" data-attr="{{ route('months.delete', $data->id) }}" title="Delete Project">

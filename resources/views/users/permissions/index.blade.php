...
<div class="row justify-content-center">
    <div class="col-lg-11 margin-tb">
        <div class="pull-left btn btn-sm btn-outline-success">
            <a href="{{ route('permissions.create') }}" class="text-light">
                        ...
                        <tbody>
                            @foreach ($permissions as $key => $permission)                            
                                <tr>
                                    ...
                                    <td class="font-weight-bold text-orange">{{ $permission->name }}</td>
                                    <td class="font-weight-bold text-blue">{{ $permission->user ? $permission->user->name : '' }}</td>
                                    <td class="font-weight-bold text-blue">{{ \App\Http\Controllers\BnumberController::toBangla($permission->created_at->format('d/m/Y')) }}</td>
                                     <td class="text-right">
                                        ...
                                        <a data-toggle="modal" id="smallButton" data-target="#smallModal" data-attr="{{ route('permissions.delete', $permission->id) }}" title="Delete Project">

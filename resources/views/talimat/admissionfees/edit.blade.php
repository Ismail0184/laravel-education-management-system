             ...
             <div class="col-lg-6 col-md-8">
                <div class="card bg-light shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5">
                          {!! Form::model($admissionfee, ['method' => 'PATCH','route' => ['admissionfees.update', $admissionfee->id]]) !!}
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    {!! Form::text('admissionfee', null, array('placeholder' => __('lang.admissionfee'),'class' => 'form-control','required'=>'required')) !!}
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group {{ $errors->has('classes_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control" name="classes_id" data-toggle="select" required>
                                        ...
                                        @foreach ($classes as $key => $value)
                                                <option value="{{ $key }}" {{ ( $key == $admissionfee->classes_id) ? 'selected' : '' }}> 
                                                    {{ $value }} 
                                                </option>
                                            @endforeach    
                                        </select>

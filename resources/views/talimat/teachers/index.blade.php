...
<div class="row justify-content-center pt-5">
    <div class="col-lg-11">
        <div class="col-lg-6 col-5 text-right">
            <a href="{{ route('teachers.index') }}" class="btn btn-sm btn-neutral">{{ __('lang.refresh') }}</a>
            <a href="{{ route('teachers.create') }}" class="btn btn-sm btn-neutral">{{ __('lang.addnew') }}</a>
            <a href="{{ route('teachers.report') }}" class="btn btn-sm btn-neutral">{{ __('lang.report') }}</a>
            <a href="{{ route('teachers.mobile') }}" class="btn btn-sm btn-neutral">{{ __('lang.mobile') }}</a>
        </div>
    @if (count($errors) > 0)
            ...
            <table class="table align-items-center table-flush">
                <thead class="thead-light">
                    <tr >
                        ...
                        <th scope="col" class="text-blue w-10">{{ __('lang.nid_image_name') }}</th>
                    ...
                    <tr>
                        ...
                        <td class="font-weight-bold text-orange">
                            @if($data->profile_image_name)
                                <a href="#" class="pop mr-2">
                                    <img src="{{ asset('teachers') }}/{{ $data->profile_image_name }}" style="height: 30px; width: 30px;">
                                </a>
                            @endif    
                            {{ $data->name }}
                        </td>
                        <td class="font-weight-bold text-orange">
                            @if($data->nid_image_name)
                                <a href="#" class="pop mr-2">
                                    <img src="{{ asset('teachers') }}/{{ $data->nid_image_name }}" style="height: 30px; width: 30px;;">
                                </a>
                            @endif
                        </td>
                        <td class="font-weight-bold text-orange">{{ $data->fname }}</td>
                        <td class="font-weight-bold text-blue">{{ $data->mobile }}</td>
                        <td class="font-weight-bold text-blue">{{ $data->email }}</td>
                        <td class="font-weight-bold text-blue">{{ $data->user->name }}</td>
                        <td class="text-right">
                            <a href="{{ route('teachers.show', $data->id) }}" title="show">
                           ...
                           <a href="{{ route('teachers.edit', $data->id) }}">
                            ...
                            <a data-toggle="modal" id="smallButton" data-target="#smallModal" data-attr="{{ route('teachers.delete', $data->id) }}" title="Delete Project">

...
<div class="container pb-5">
        ...
        <div class="row justify-content-center">
           <div class="col-xs-12 col-sm-12 col-md-12 text-center">
                <a href="{{ route('exambenches.index') }}" class="btn btn-sm btn-neutral">{{ __('lang.back') }}</a>
           </div>
             <div class="col-lg-6 col-md-8">
                <div class="card bg-light shadow border-0">
                    ...
                    <div class="card-body px-lg-5 py-lg-5">
                        {!! Form::model($exambench, ['method' => 'PATCH','route' => ['exambenches.update', $exambench->id]]) !!}
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    {!! Form::text('name', null, array('placeholder' => __('lang.exambench'),'class' => 'form-control','required'=>'required')) !!}
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    {!! Form::text('bench_row', null, array('placeholder' => __('lang.seat_count'),'class' => 'form-control','required'=>'required')) !!}
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group {{ $errors->has('examholl_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control" name="examholl_id" data-toggle="select" required>
                                        ...
                                        @foreach ($examholls as $key => $value)
                                                <option value="{{ $key }}" {{ ( $key == $exambench->examholl_id) ? 'selected' : '' }}> 
                                                    {{ $value }} 
                                                </option>
                                            @endforeach    
                                        </select>
                                ...
                                <div class="form-group {{ $errors->has('examname_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control" name="examname_id" data-toggle="select" required>
                                        ...
                                        @foreach ($examname as $key => $value)
                                                <option value="{{ $key }}" {{ ( $key == $exambench->examname_id) ? 'selected' : '' }}> 
                                                    {{ $value }} 
                                                </option>
                                            @endforeach    
                                        </select>
